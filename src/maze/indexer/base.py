...
class TestCase:
    ...
    test_function: str
    file_path: str
    command: str | None = None  # Command to run this test
...
class IndexingResult:
    ...
    def to_indexed_context(self, language: str, project_path: str | None = None) -> IndexedContext:
        ...
        return IndexedContext(
            ...
            tests=[
                {"name": t.name, "kind": t.kind, "cmd": t.command or f"test {t.name}"}
                for t in self.tests
            ],
...
class BaseIndexer(ABC):
    ...
    def extract_imports(self, content: str, file_path: Path) -> list[ImportInfo]:
        ...
        for i, line in enumerate(lines):
            ...
            if line.startswith("import "):
                ...
                imports.append(
                    ImportInfo(
                        module=parts.split()[0], symbols=[], file_path=str(file_path), line=i + 1
                    )
                )
                ...
                if " import " in line:
                    ...
                    imports.append(
                        ImportInfo(
                            module=module, symbols=symbols, file_path=str(file_path), line=i + 1
                        )
                    )
    ...
    def extract_patterns(self, symbols: list[Symbol]) -> list[str]:
        ...
        async_count = sum(
            1 for name in function_names if name.startswith("async") or "Async" in name
        )
        ...
        test_count = sum(
            1 for name in function_names if "test" in name.lower() or "spec" in name.lower()
        )
        if test_count > 0:
        ...
        getter_count = sum(
            1 for name in function_names if name.startswith("get") or name.startswith("set")
        )
    ...
    def find_files(self, directory: Path, extensions: set[str] | None = None) -> list[Path]:
        """
