...
from maze.synthesis.grammars.python import PYTHON_FUNCTION, PYTHON_FUNCTION_BODY
from maze.synthesis.grammars.typescript import TYPESCRIPT_FUNCTION_BODY
...
class TestGrammarApplicationVerification:
    ...
    def test_grammar_is_sent_to_provider(self):
        ...
        assert hasattr(pipeline, "_last_grammar"), "Pipeline should track last grammar used"

        assert (
            pipeline._last_grammar is not None
        ), "Grammar should be non-None when constraints enabled"

        assert (
            len(pipeline._last_grammar) > 100
        ), f"Grammar suspiciously short ({len(pipeline._last_grammar)} chars)"

        # Verify it's a Lark grammar
        assert (
            "?start:" in pipeline._last_grammar or "start:" in pipeline._last_grammar
    ...
    def test_no_grammar_when_disabled(self):
        ...
        if hasattr(pipeline, "_last_grammar"):
            assert (
                pipeline._last_grammar is None or pipeline._last_grammar == ""
...
class TestMultiLanguageCorrectness:
    ...
    def test_language_compilation(self, language, prompt, checker):
        ...
        assert pipeline._last_grammar is not None, f"Grammar should be applied for {language}"
...
"""
        ...
        print("\nResults:")
        print(
            f"  Constrained:   {results['constrained_valid']}/{results['constrained_total']} ({constrained_rate:.0%})"
        )
        print(
            f"  Unconstrained: {results['unconstrained_valid']}/{results['unconstrained_total']} ({unconstrained_rate:.0%})"
        )
        print(f"  Improvement:   {(constrained_rate - unconstrained_rate):.0%}")

        # Core value proposition test
        assert (
            constrained_rate >= 0.90
        ), f"Constrained should achieve ≥90% validity, got {constrained_rate:.0%}"

        assert (
            constrained_rate > unconstrained_rate
...
"""
        ...
        print("\n📊 Comparison Results:")
        print(
            f"  Constrained:   {results['constrained']['valid']}/{results['constrained']['total']} ({constrained_rate:.0%})"
        )
        print(
            f"  Unconstrained: {results['unconstrained']['valid']}/{results['unconstrained']['total']} ({unconstrained_rate:.0%})"
        )
        print(f"  Improvement:   {(constrained_rate - unconstrained_rate):.0%}")
